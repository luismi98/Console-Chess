// Luismi 22 August 2022
// Re-wrote the main.cpp grouping the code into functions to aid readability
// Also as practise to remind myself how to code in C++
...
void game_over_print(const bool& check, const piece_colour& colour_turn) {
    if (check) {
...
void test_check_print(const bool& check, const piece_colour& colour_turn) {
    if (check) {
...
void print_all_allowed_moves(const std::unique_ptr<board>& gameboard, const font& font_choice, const piece_colour& colour_turn, 
                            const bool& check, const std::list<std::string> all_allowed_moves) {
    system("cls"); //clear the console 
    gameboard->show(font_choice, colour_turn, check);
    test_check_print(check, colour_turn);
...
int main() {
    ...
    font font_choice{ get_font_choice() };
    ...
    while (!game_over) {
        ...
        catch (const std::exception& all_allowed_err) {
            std::cerr << all_allowed_err.what() << std::endl;
            return 1;
        }
        if (all_allowed_moves.size() == 0) {
            game_over = true;
            game_over_print(check, colour_turn);
            continue;
        }
        test_check_print(check, colour_turn);
        ...
        while (!valid_input) {
            ...
            bool show_all_allowed_moves{ false };
            bool valid_input_format{ false };
            valid_input_format = check_input_format(input_string, got_all_allowed_moves, show_all_allowed_moves);
            if (!valid_input_format) {
                continue;
            }
            else if (show_all_allowed_moves) {
                print_all_allowed_moves(gameboard, font_choice, colour_turn, check, all_allowed_moves);
            }

            

            bool check_input_format(const std::string& input_string, const bool& got_all_allowed_moves, bool& show_all_allowed_moves) {
                if ()
